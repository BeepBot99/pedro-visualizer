...
<div class="bg-base-300 rounded-lg p-4 pb-0 my-2">
  ...
  {#if expanded}
    <div bind:this={collapsible}
      ...
      <div class="flex justify-between items-center my-4">
        <span>Anchor</span>
        <div class="flex gap-2">
          {#if waypoint.handleBefore && waypoint.handleAfter}
            {#if waypoint.smooth}
              <ButtonUI onclick={() => waypoint.smooth = false} name="Unlock Handles"
                        Class="text-base-content/50 p-1! m-0!">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                     stroke="currentColor" class="size-6">
                  <path stroke-linecap="round" stroke-linejoin="round"
  ...
  <div class="flex items-center justify-center">
    <ButtonUI onclick={onAddAfter} Class="m-4 border-base-content! hover:bg-base-content/10" name="Add Waypoint After">
      Add Waypoint After
    </ButtonUI>
  </div>
</div>