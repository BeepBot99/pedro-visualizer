<script lang="ts">
  ...
  import { cubicBezierChainLength, pathChainToCubicBezierChain } from "$lib/bezier";
  ...
  const PATH_DURATION_SCALAR = 0.005;
  const PATH_DURATION_MIN = 2;
  ...
  $effect(() => {
    ...
    timeManager.timeline.to(
      robot,
      {
        ...
        duration: PATH_DURATION_SCALAR * cubicBezierChainLength($state.snapshot(cubicBezierChain)) + PATH_DURATION_MIN,
        ease: "sine.inOut"
      },
      0
    );
